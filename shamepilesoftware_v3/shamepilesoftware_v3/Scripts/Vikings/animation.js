var game = (function () {
	...
	function beginClock() {
		debug.showCallstack("beginClock");
	    ...
	    isWon = false;
	    //debug.writeLine("Initialized game variables");

	    // Initialize the game character.
	    character = new Sprite(10, 200, 50, 78);
	    //debug.writeLine("Sprite created");
	...
	function draw() {
		//debug.showCallstack("draw");
		
		// Check to see if we have the canvas
		// and drawing context.
		if ((!canvas) ||
			(!ctx)) {
			canvas = dom.getNode("canvas");
			ctx = canvas.getContext("2d");
			//debug.writeLine("Context is: " + (typeof ctx));
		}
	...
	function updateCoords() {
	    ...
	    if ((!xCoords) ||
            (!yCoords)) {
	        xCoords = dom.getNode("x-coord");
	        yCoords = dom.getNode("y-coord");
	    }
